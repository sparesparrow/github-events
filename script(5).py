# Create the core GitHub Events collector module
core_collector_code = '''"""
...
class GitHubEventsCollector:
    ...
    async def get_event_counts_by_type(self, offset_minutes: int) -> Dict[str, int]:
        ...
        async with aiosqlite.connect(self.db_path) as db:
            ...
            # Initialize all monitored event types to 0
...
with open("github_events_monitor/collector.py", "w") as f:
    f.write(core_collector_code)

print("✅ Created github_events_monitor/collector.py")
print(f"📊 Size: {len(core_collector_code)} characters")
print("\nCore module features:")
print("- Async GitHub Events API client with rate limiting")
print("- SQLite database with optimized indices")
print("- Event filtering for WatchEvent, PullRequestEvent, IssuesEvent")
print("- Metrics: average PR intervals, event counts by type, trending repos")
